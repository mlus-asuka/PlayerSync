...
public class MixinNbtBackedPlayerData {
        ...
        if(playerData instanceof PokedexManager){
            ...
            try {
                JDBCsetUp.executePreparedUpdate(
                        "INSERT INTO " + Tables.cobblemon() + " (uuid, pokedex) VALUES (?, ?) " +
                                "ON DUPLICATE KEY UPDATE pokedex = VALUES(pokedex)",
                        playerData.getUuid().toString(), serializedData);
    ...
    private void load$playerSync(UUID uuid, CallbackInfoReturnable<InstancedPlayerData> cir){
        ...
        try (JDBCsetUp.QueryResult qr = JDBCsetUp.executePreparedQuery(
                "SELECT pokedex FROM " + Tables.cobblemon() + " WHERE uuid = ?", uuid.toString())) {
